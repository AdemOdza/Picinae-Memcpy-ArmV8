...
def parse_cycle_counts(log_path):
    ...
    for line in lines:
        line = line.strip()
        if line.startswith("=== Test: vListInitialise (reinit) ==="):
            capture = True
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Compare vListInitialise cycle counts with expected timing equation.")
    ...
    plot_comparison("vListInitialise", "Iteration", "Cycle Count",
                    [('Expected Range (min-max)', 'lightgray', min_expected_vals[:PLOT_LEN], max_expected_vals[:PLOT_LEN])],
                    [('Measured', measured_vals[:PLOT_LEN])], savepath="./plots/vListInitialise.png")
