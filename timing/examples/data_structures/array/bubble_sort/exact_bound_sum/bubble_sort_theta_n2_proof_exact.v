...
Lemma mul_sub1 : forall y x,
    0 < y ->
    (y - 1) * x + x = x * y.
Proof.
    induction y using N.peano_ind. lia.
    intros. replace (N.succ y - 1) with y by lia.
    rewrite N.mul_succ_r. lia.
Qed.
...
Definition time_of_bubble_sort_theta_n2 (len : N) (N_swaps : N) (t : trace) :=
  cycle_count_of_trace t =
    ...
    (* inner loop full iterations with swaps *)
    N_swaps * (
        ttbne +
        tlw + tlw +
        tfbgeu + tsw + tsw + 
    ...
    (* inner loop full iterations without swaps *)
    (len * (len - 1) - N_swaps) * (
        ttbne +
        tlw + tlw +
        ttbgeu +
...
| 0x220 => Some (exists N_swaps, time_of_bubble_sort_theta_n2 len N_swaps t)
...
Proof using.
    ...
    (* 0x1f0 -> *)
    destruct PRE as (mem & a4 & N_swaps & MEM & A0 & A1 & A3 & A4 & A4_VALID & 
        ARR_VALID & Cycles).
    repeat step.
        (* a4 = len *)
        exists N_swaps.
    ...
    destruct PRE as (mem & a4 & inner_loop_count & N_swaps & N_swaps_this_iter & 
                     ...
                     & A4_LEN & ARR_VALID & Cycles).
    step.
        (* not at end of array, continue inner loop *)
        repeat step.
        (* arr[j-1] < arr[j] *)
        do 2 eexists; exists (1 + inner_loop_count), (1 + N_swaps), (1 + N_swaps_this_iter); 
                repeat split; eauto.
